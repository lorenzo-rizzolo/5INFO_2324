...
struct info_dati socket_receive(int socket_fd, char *buf)
{
    int msg_size;
    struct sockaddr_in clientaddr; /* client address */
    socklen_t client_struct_len; /* lunghezza dell'indirizzo del client */
    struct info_dati sent;
    ...
    sent.port = ntohs(clientaddr.sin_port);
    sent.indirizzo = inet_ntoa(clientaddr.sin_addr);
    sent.buf_leght = strlen(buf);
...
int main(int argc, char **argv) 
{
    ...
    for(;;) {
        sent = socket_receive(socket_fd, buf);
        
        printf("UDP server ha ricevuto %d byte: %s\n", sent.buf_leght, buf);
        
        byte_sent = socket_send(socket_fd, sent.indirizzo, sent.port, buf);    

        printf("Inviato %d bytes con successo a %s alla porta %d\n", byte_sent, sent.indirizzo, htons(sent.port));
    }
}