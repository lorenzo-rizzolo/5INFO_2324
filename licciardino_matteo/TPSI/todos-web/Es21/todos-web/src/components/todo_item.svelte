...
<Cell> <input type="text" class="todo-item-input-text {todo.done == true ? 'text-done' : ''}" id="{todo.id}" placeholder="Inserisci in nuovo ToDo" bind:value={todo.task} on:change={edit_task}></Cell>
<Cell> <Priority disabled={todo.done} bind:prio={todo.priority} /> </Cell>
<Cell last> <Icon name="delete_forever" handler={() => item_change('delete')} color="red"/> </Cell>
