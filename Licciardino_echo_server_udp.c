...
int socket_receive(int socket_fd, char *buf, struct sockaddr_in *client_addr)
{
    int msg_size;
...
{
    struct sockaddr_in client_address; /* indirizzo e porta client */  
    int byte_sent;

    /* prepara le informazioni sulla destinazioe del datagram */
    client_address.sin_family = AF_INET;
    client_address.sin_port = clientaddr.sin_port;
    client_address.sin_addr.s_addr = clientaddr.sin_addr.s_addr;
...
int main(int argc, char **argv) 
{
    ...
    for(;;) {
        ...
        msg_size2 = socket_send(socket_fd, client_addr, buf);
        printf("Inviato %d bytes con successo a %s\n", msg_size2, inet_ntoa(client_addr.sin_addr));
    }
}