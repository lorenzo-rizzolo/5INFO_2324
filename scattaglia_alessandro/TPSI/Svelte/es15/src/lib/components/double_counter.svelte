<script>
   ...
   import {store_counter1 } from '../js/store.js';
    const dispatch = createEventDispatcher();
    export let contatore1 = 0;

    function counter_incr() {
        contatore1 += 2;
        $store_counter1 = contatore1;
        emit_event("incr");
    }
    ...
    function emit_event(tipo) {
        dispatch("contatore1", { tipo: tipo, valore: contatore1});
    }
</script>
...
<svelte:head>
    ...
    />
    <link
        href="https://fonts.googleapis.com/css2?family=Bungee+Spice&family=Lexend&display=swap"
        rel="stylesheet"
    />
</svelte:head>
<main>
